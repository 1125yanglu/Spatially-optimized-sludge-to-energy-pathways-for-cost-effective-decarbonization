...
params = {
    'eR1': {'min': 118.94, 'mode': 148.68, 'max': 178.42},  # kWh t-1 Sludge
    'eR2': {'min': 4.58, 'mode': 5.72, 'max': 6.86},  # kWh t-1 Sludge
    'eR3': {'min': 394.66, 'mode': 493.33, 'max': 592.00},  # kWh t-1 Sludge
}
# Monte Carlo simulation times
num_simulations = 1000


def Energy(sd, r, eR1, eR2, eR3):
    r1, r2, r3 = r[:, 0], r[:, 1], r[:, 2]
    Power = np.sum(sd * (eR1 * r1 + eR2 * r2 + eR3 * r3) * 365 / 1e9)  # energy recovery, TWh/year
    return Power


def Output(sd, dR, eR1, eR2, eR3):
    rs = np.zeros((31, 3))  # The proportion of R1, R2, R3 and R4 routes from 2024 to 2060
    rs[:, 0] = dR['R1'].to_numpy()
    rs[:, 1] = dR['R2'].to_numpy()
    rs[:, 2] = dR['R3'].to_numpy()
    power = np.zeros(num_simulations)
    for p in range(num_simulations):
        power[p] = Energy(sd, rs, eR1[p], eR2[p], eR3[p])
    return power
...
for i in range(37):
    CE_future[i] = Output(dd[i], dd_Rs, E_R1, E_R2, E_R3)
